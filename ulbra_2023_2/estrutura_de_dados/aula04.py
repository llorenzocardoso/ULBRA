...
def remover_varios(voltas, uma_pilha):
    if not uma_pilha:
        print("A pilha está vazia")
    else: 
        if voltas > len(uma_pilha):
            voltas = len(uma_pilha)
        for _ in range(voltas):
            if not uma_pilha:
                print("A pilha está vazia")
            else:
                push(deletados, uma_pilha[-1])
                pop(uma_pilha)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if opc == 1:
            ...
            voltas = int(input("Quantos numeros você quer desempilhar? "))
            remover_varios(voltas, pilha);
        elif opc == 0:
            break
        else:
